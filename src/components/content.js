		...
		return (
			<div className="card__content">
				<p>
					...
					{this.renderLabel(data.pluralNoun)} and the 
					...
					{this.renderLabel(data.adjectiveTwo)} ball that just cut the inside of the 
					{this.renderLabel(data.nounOne)} for a strike. That makes the count 
					{this.renderLabel(data.numberOne)} strikes and 
					{this.renderLabel(data.numberTwo)} balls. Now there's the next pitch. The batter swings and connects. It's a long, high 
					...
					{this.renderLabel(data.adjectiveFour)}-stop. Here's the pitch... and it's hit... a short ground ball to third 
					{this.renderLabel(data.nounThree)}. 
					{this.renderLabel(data.celebFour)} scoops it up and throws it to first base for an out, and the game is over. And the Yankees move into second place in the 
